...
const H5Control = (function() {
    ...
    let state = { idx: 0, active: false, timer: null, pTimer: null, liked: false, collected: false };
    ...
    const startCountdown = () => {
        const updateTimer = () => {
            ...
            if (diff <= 0) {
                document.getElementById('timerMain').innerHTML = "<div style='color:#FE2C55; font-weight:900;'>LIVE NOW</div>";
                return;
            }
    ...
    const stopAll = () => { clearTimeout(state.timer); clearInterval(state.pTimer); };
    const next = () => { state.idx = (state.idx + 1) % DATA.images.length; render(); };
    const prev = () => { state.idx = (state.idx - 1 + DATA.images.length) % DATA.images.length; render(); };
    ...
    const share = () => { alert("Link copied to clipboard!"); };
